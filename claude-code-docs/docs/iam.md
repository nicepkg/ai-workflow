...
- [Claude for Teams or Enterprise](/docs/en/setup#for-teams-and-organizations) (recommended)
- [Claude Console with team billing](/docs/en/setup#for-teams-and-organizations)
...
For a quick reference on permission rule syntax including wildcards, see [Permission rule syntax](/docs/en/settings#permission-rule-syntax) in the settings documentation.
...
| `dontAsk` | Auto-denies tools unless pre-approved via `/permissions` or [`permissions.allow`](/docs/en/settings#permission-settings) rules |
...
Add these rules to the `deny` array in your [settings](/docs/en/settings#permission-settings) or use the `--disallowedTools` CLI flag to disable specific agents. For example, to disable the Explore agent:
...
For organizations that need centralized control over Claude Code configuration, administrators can deploy `managed-settings.json` files to [system directories](/docs/en/settings#settings-files). These policy files follow the same format as regular settings files and cannot be overridden by user or project settings.
