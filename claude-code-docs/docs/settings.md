...
| `deny` | Array of permission rules to deny tool use. Use this to exclude sensitive files from Claude Code access. See [Permission rule syntax](#permission-rule-syntax) and [Bash permission limitations](/docs/en/iam#tool-specific-permission-rules) | `[ "WebFetch", "Bash(curl:*)", "Read(./.env)", "Read(./secrets/**)" ]` |
...
For detailed information about tool-specific permission patterns—including Read, Edit, WebFetch, MCP, Task rules, and Bash permission limitations—see [Tool-specific permission rules](/docs/en/iam#tool-specific-permission-rules).
...
- [Identity and Access Management](/docs/en/iam#configuring-permissions) - Permission system overview and how allow/ask/deny rules interact
- [Tool-specific permission rules](/docs/en/iam#tool-specific-permission-rules) - Detailed patterns for Bash, Read, Edit, WebFetch, MCP, and Task tools, including security limitations
- [Managed settings](/docs/en/iam#managed-settings) - Managed policy configuration for organizations
